...
double distancia(const std::pair<double, double>& orig, const std::pair<double, double>& dest) {
    auto const R = 6371.0; // raio médio da Terra em Km
    auto const PI = 3.14159265; // constante PI
...
auto& roteiro_menor_distancia(std::map<std::string, std::vector<std::pair<double, double>>>& roteiros) {
    auto it_menor = roteiros.begin();
    double menor_distancia = std::numeric_limits<double>::max();
    for (auto it = roteiros.begin(); it != roteiros.end(); ++it) {
        double distancia_total = 0.0;
        auto origem = it->second.begin();
        auto destino = origem + 1;
        while (destino != it->second.end()) {
...
int main() {
    ...
    auto& roteiro_menor_distancia(roteiros);

    for (std::map<std::string, std::vector<std::pair<double, double>>>::const_iterator it = roteiros.begin(); it != roteiros.end(); ++it) {
